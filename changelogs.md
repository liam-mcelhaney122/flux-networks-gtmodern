...
* Add a per-network energy type (FE or EU). An injected `IEnergySystem`
  does all boundary conversion (1 EU = 4 FE), so the network's internal
  math stays in one energy type at a time
* Expose GT energy containers on flux plugs and points, behind
  `enableGTCEU` and only when GTCEU is installed. This lets GT cables and
  generators connect directly
* Harden the GTCEU connector. It now accepts whole amps only, advertises
  honest headroom so generators idle at zero demand, and uses overflow-safe
  math for amps and voltage
* Re-denominate each device's buffer and limit when its network's energy
  type changes. This includes a device that reconnects from an unloaded
  chunk
